...
{
	protected Dictionary<T,E> map = new Dictionary<T, E>();
	protected E stateID;
	public E ID { get { return stateID; } }
	public T	nullTransition;
	public E	nullStateID;
	
	public void AddTransition(T trans, E id)
	{
		// Check if anyone of the args is invalid
			if ((int)Convert.ChangeType(trans, trans.GetTypeCode()) ==(int)Convert.ChangeType(nullTransition, nullTransition.GetTypeCode()))
		{
			Debug.LogError("FSMState ERROR: NullTransition is not allowed for a real transition");
			return;
		}
		
		if ((int)Convert.ChangeType(id, id.GetTypeCode()) ==(int)Convert.ChangeType(nullStateID, nullStateID.GetTypeCode()))
		{
	...
	/// </summary>
	public E GetOutputState(T trans)
	{
...
/// </summary>
public class FSMSys<T,E>
	where T:struct,IConvertible
	where E:struct,IConvertible
{
	private List<State<T,E>> states;
	
	// The only way one can change the state of the FSM is by performing a transition
	// Don't change the CurrentState directly
	private E currentStateID;
	public 	E CurrentStateID { get { return currentStateID; } }
	private State<T,E> currentState;
	public 	State<T,E> CurrentState { get { return currentState; } }
	public 	E nullStateID;
	public 	T nullTransition;

	public FSMSys()
	{
		states = new List<State<T,E>>();
	}
	...
	/// </summary>
	public void AddState(State<T,E> s)
	{
		...
		foreach (State<T,E> state in states)
		{
			if ((int)Convert.ChangeType(state.ID, state.ID.GetTypeCode()) ==(int)Convert.ChangeType(s.ID, s.ID.GetTypeCode()))
			{
	...
	public void DeleteState(E id)
	{
		...
		foreach (State<T,E> state in states)
		{
			if ((int)Convert.ChangeType(state.ID, state.ID.GetTypeCode()) ==(int)Convert.ChangeType(id, id.GetTypeCode()))
			{
				states.Remove(state);
				return;
			}
	...
	public void PerformTransition(T trans)
	{
		...
		E id = currentState.GetOutputState(trans);
		if ((int)Convert.ChangeType(id, id.GetTypeCode()) ==(int)Convert.ChangeType(nullStateID, nullStateID.GetTypeCode()))
		{
		...
		foreach (State<T,E> state in states)
		{
			...
			if ((int)Convert.ChangeType(state.ID, state.ID.GetTypeCode()) ==(int)Convert.ChangeType(currentStateID, currentStateID.GetTypeCode()))
			{
